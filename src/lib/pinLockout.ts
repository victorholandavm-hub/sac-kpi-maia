...
export async function recordFailedPinAttempt(table: "assemblers" | "stores", idColumn: string, idValue: string): Promise<void> {
  const admin = getSupabaseAdmin();
...
export async function resetPinAttempts(table: "assemblers" | "stores", idColumn: string, idValue: string): Promise<void> {
  const admin = getSupabaseAdmin();
