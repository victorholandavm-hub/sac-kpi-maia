...
export async function recordFailedPinAttempt(table: "assemblers" | "gerentes", idColumn: string, idValue: string): Promise<void> {
  const admin = getSupabaseAdmin();
...
export async function resetPinAttempts(table: "assemblers" | "gerentes", idColumn: string, idValue: string): Promise<void> {
  const admin = getSupabaseAdmin();
