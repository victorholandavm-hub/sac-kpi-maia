...
// Free vs Pro): esta função varria totvs_order_items INTEIRO join
// totvs_orders (305 mil+ linhas, cresceu muito com o backfill completo do
// TOTVS) + totvs_delivery_cargas (pra achar a data de entrega real), e
// categorizava/agregava tudo em JS. Agora consulta
// v_recompra_categorias_por_cliente (migration 0123), que já faz a
// categorização por palavra-chave, a preferência por data de entrega real
// (mesma regra de antes -- ver comentário na própria view) e o
// agrupamento por cliente×categoria dentro do Postgres -- só a linha já
// agregada de cada combinação trafega pela rede, não os pedidos/itens
// crus. fetchDeliveryDatePorInvoiceSerie (scan à parte de
// totvs_delivery_cargas) deixou de existir -- virou um CTE dentro da view.
//
// ATENÇÃO: a categorização por palavra-chave está DUPLICADA na view SQL
// (CATEGORIAS_CICLO abaixo só serve mais pra achar o cicloMeses/label de
// cada key, não pra categorizar de novo) -- mudou uma lista, muda a outra
// (mesmo espírito de RESOLVIDO_LABELS em totvsSync.ts/entregasRisco.ts,
// já documentado como duplicação aceita). Ver comentário completo na
// migration 0123_views_agregacao_clientes_recompra.sql.
  ...
  const rows = await fetchAllPagesParallel<CategoriaAgregadaRow>(
    (from, to) =>
      admin
        ...
        .select("client_id, categoria, ultima_data, entrega_confirmada, valor_total, qtd_compras", { count: "exact" })
