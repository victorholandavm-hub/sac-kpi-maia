...
export type EncomendaRequester =
  | { kind: "caixa"; storeId: string }
  | { kind: "vendedor"; storeId: string; name: string }
  | { kind: "gerente"; storeIds: string[]; name: string };

// Quem pode lançar/ver encomenda de uma loja: caixa (PIN por loja), vendedor
// (PIN por pessoa, 1 loja) ou gerente (login que ele já usa em
// /assistencia/loja, cookie com path /assistencia então já chega aqui sem
// logar de novo). Tenta cada sessão em sequência, mesmo padrão de
// requireEncomendaActor (src/lib/encomendaAuth.ts) só que pro lado de quem
// solicita, não de quem processa (CD/fábrica/admin/assistência).
export async function resolveEncomendaRequester(): Promise<EncomendaRequester | null> {
  const caixaStoreId = await getCaixaSession();
  if (caixaStoreId) return { kind: "caixa", storeId: caixaStoreId };
