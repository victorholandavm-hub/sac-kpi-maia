  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-sm p-4">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-xs font-mono font-semibold text-gray-500 dark:text-gray-400">#{request.ticketNumber}</span>
        <StatusBadge status={request.status} showInfo />
        <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">{REQUEST_TYPE_LABELS[request.type] ?? request.type}</span>
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400">· {request.storeName}</span>
      </div>

      <p className="text-sm font-medium text-gray-800 dark:text-gray-100">{request.clientName ?? "Sem nome de cliente"}</p>
      {request.assemblerName ? <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Montador: {request.assemblerName}</p> : null}
