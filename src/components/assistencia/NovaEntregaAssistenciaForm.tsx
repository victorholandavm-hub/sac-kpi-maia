...
export function NovaEntregaAssistenciaForm({
  ...
  function runClientLookup(code: string) {
    ...
    withRetry(() => lookupTotvsClientForTeam(code))
      .then((match) => {
        ...
        // os dados do cliente quando coloco o codigo dele") -- mesmo
        // comentário em SacCreateRequestForm.tsx: a maioria dos códigos só
        // bate no fallback totvs_orders (sem telefone/endereço), e a
        // mensagem genérica "Cliente encontrado" escondia isso.
  ...
  // Vincular pedido de peça já chegado -- mesmo recurso de
  // QuickCreateRequestForm.tsx (aba Visitas), estendido pra Entregas
  // 10/09/2026: "aquela mudança... só ficou na aba de visitas, precisa
  // ficar também na aba de entregas". Aqui o tipo vinculado é "Envio de
  // peça" (não "Troca de peça", que é visita de montador) -- a peça já
  // chegou e vai pro cliente pelo motorista, mesmo fluxo dessa tela.
  ...
  useEffect(() => {
    ...
    // do React Compiler) -- mesmo padrão do efeito de clientCode acima.
    ...
    if (m.clientCpf) {
      lookupTotvsClientByCpfForTeam(m.clientCpf)
        .then((match) => {
          ...
          // acima (runClientLookup) -- reaproveita o preenchimento de
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4 max-w-xl">
      ...
      >
        {/* Vincular pedido de peça já chegado -- mesmo recurso/comentário de
            QuickCreateRequestForm.tsx, ver selectPartOrder acima. Fica antes
