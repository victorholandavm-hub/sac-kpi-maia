...
import { setFinanceiroPin } from "@/app/assistencia/admin-actions";
import { useQuickAction } from "./useQuickAction";
import { PIN_LENGTH } from "@/lib/pinConfig";

export function FinanceiroPinField({ name, hasPin }: { name: string; hasPin: boolean }) {
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            ...
            run(async () => {
              await setFinanceiroPin(name, pin);
              setEditing(false);
              setPin("");
