          ...
          >
            <div className="flex flex-col items-center gap-0.5 shrink-0">
              <button
                ...
                className="text-sm leading-none px-1 disabled:opacity-25 text-gray-500 dark:text-gray-400"
              >
                ▲
              </button>
              <button
            ...
            >
              <div className="flex flex-col gap-1 min-w-0 w-0 grow">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {r.scheduledTime ? (
                    <span className="text-xs font-medium px-2 py-0.5 rounded-full border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300">
                  ...
                  {r.rota && (DELIVERY_REQUEST_TYPES as readonly string[]).includes(r.type) ? (
                    <span
                      ...
                      style={{ color: `color-mix(in srgb, ${ROTA_COLORS[r.rota]} 70%, var(--foreground))`, background: `color-mix(in srgb, ${ROTA_COLORS[r.rota]} 14%, var(--surface-1))` }}
                    >
                      {ROTA_LABELS[r.rota]}
                      {r.rotaExceptionNote ? " ⚠" : ""}
                    </span>
                  ...
                  <span
                    ...
                    style={{
                      color: `color-mix(in srgb, ${REQUEST_TYPE_COLORS[r.type] ?? "#6B7280"} 70%, var(--foreground))`,
                      background: `color-mix(in srgb, ${REQUEST_TYPE_COLORS[r.type] ?? "#6B7280"} 14%, var(--surface-1))`,
                    }}
                  >
                    {REQUEST_TYPE_LABELS[r.type] ?? r.type}
                  </span>
                  <span className="text-xs text-gray-400 dark:text-gray-500">{r.storeName}</span>
                </div>
                <p className="text-sm truncate">
                  {/* Negrito + caixa alta -- pedido do Victor 25/08/2026
                      ...
                      -- alinhado agora). */}
                  <span className="font-bold uppercase text-gray-800 dark:text-gray-100">{r.clientName ?? "Sem nome de cliente"}</span>
