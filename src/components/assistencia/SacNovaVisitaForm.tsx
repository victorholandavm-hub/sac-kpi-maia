...
type SacVisitaType = "montagem" | "desmontagem";
...
// desmontar na mesma visita) -- mesmo desenho de SacCreateRequestForm.tsx.
...
// "Nova visita" do SAC -- só o intake de montagem/desmontagem (pedido do
// Victor 18/08/2026: "SAC também faz desmontagem" + separação visita x
// entrega). Sem montador aqui -- quem atribui é a assistência depois (mesma
// regra de sempre, ver comentário de createSacRequest em actions.ts). Sem
// rota/autorizado por/quem errou -- isso é entrega, não visita.
export function SacNovaVisitaForm({ stores }: { stores: Store[] }) {
  const [state, formAction, pending] = useActionState<FormState, FormData>(createSacRequest, undefined);
  const [type, setType] = useState<SacVisitaType>("montagem");
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4 max-w-xl">
      <FormSection title="Tipo e loja" number={1}>
        <Field label="Tipo">
          <select
            ...
            value={type}
            onChange={(e) => {
              setType(e.target.value as SacVisitaType);
              setCombo(false);
            }}
          ...
          >
            ...
            <option value="desmontagem">{REQUEST_TYPE_LABELS.desmontagem}</option>
          </select>
        </Field>
      ...
      <FormSection
        title={combo ? `Móveis a ${type === "montagem" ? "montar" : "desmontar"}` : type === "montagem" ? "Móvel(is) a montar" : "Móvel(is) a desmontar"}
        number={3}
      ...
      {combo ? (
        <FormSection title={`Móveis a ${type === "montagem" ? "desmontar" : "montar"}`} number={4} hint="Código do produto obrigatório.">
          <ItemsFields
      ...
      <FormSection title="Detalhes" number={5} hint="Conte o que precisa ser feito, com o máximo de detalhe que puder.">
        <Field label="Motivo *">
          <textarea name="reason" rows={2} required placeholder="Ex: cliente comprou um roupeiro novo" className="rounded border px-3 py-2" style={inputStyle} />
        </Field>
      </FormSection>
