...
import { EquipeVendedorPinField } from "@/components/assistencia/EquipeVendedorPinField";
...
export default async function LojaEquipePage() {
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto p-6 flex flex-col gap-6 w-full min-w-0">
      <AssistenciaHeader title="Equipe da loja" subtitle="Cadastre e libere acesso pra caixa e vendedor lançarem encomenda" />
      ...
      >
        ...
        <p className="text-xs" style={{ color: "var(--text-muted)" }}>
          Cada vendedor tem seu próprio PIN e entra com nome + PIN em{" "}
          <span className="font-mono">/assistencia/encomendas/vendedor/login</span>.
        </p>
        <ul className="flex flex-col gap-2">
          {vendedores.map((v) => (
            <li key={v.name}>
              <EquipeVendedorPinField name={v.name} storeName={v.storeName} hasPin={v.hasPin} ativo={v.ativo} />
            </li>
          ))}
          {vendedores.length === 0 ? (
            <li className="text-sm" style={{ color: "var(--text-muted)" }}>
